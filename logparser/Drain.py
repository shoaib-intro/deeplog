...
class LogParser:
    ...
    def outputResult(self, logClustL):
        ...
        df_event[["EventId", "EventTemplate", "Occurrences"]].to_pickle(os.path.join(self.savePath, self.logName + '_templates.pkl'))
    ...
    def generate_logformat_regex(self, logformat):
        ...
        for k in range(len(splitters)):
            if k % 2 == 0:
                splitter = re.sub(' +', '\\\s+', splitters[k]) # change from \s+ to \\\s+
                regex += splitter
            else:
    ...
    def get_parameter_list(self, row):
        ...
        template_regex = re.sub(r'\\ +', r'\\s+', template_regex) # changed from template_regex = re.sub(r' +', r'\s+', template_regex)
        template_regex = "^" + template_regex.replace("\<\*\>", "(.*?)") + "$"
        parameter_list = re.findall(template_regex, row["Content"])
        parameter_list = parameter_list[0] if parameter_list else ()
        parameter_list = list(parameter_list) if isinstance(parameter_list, tuple) else [parameter_list]
        return parameter_list
# two changes required at line 327 and 340 
